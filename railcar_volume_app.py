...
# Define full height (inches) and volume (gallons) data from chart
data = [
    ...
    (25.0, 5129), (30.0, 6546), (35.0, 8035), (40.0, 9580), (45.0, 11166), (50.0, 12780), (55.0, 14408),
    (60.0, 16037), (65.0, 17655), (70.0, 19248), (75.0, 20802), (80.0, 22303), (85.0, 23736), (90.0, 25082),
    (95.0, 26320), (100.0, 27424), (105.0, 28399), (110.0, 29058), (114.0, 29340)
]

height_in, volume_gal = zip(*data)

# DEBUG: Check lengths before interpolation
try:
    st.write(f"⚠️ Length of height_in: {len(height_in)}")
    st.write(f"⚠️ Length of volume_gal: {len(volume_gal)}")
except Exception as e:
    st.error(f"Error checking lengths: {e}")

# Only proceed with interpolation if data lengths match
if len(height_in) == len(volume_gal):
    ...
    if mode == "Height (cm)":
        ...
        target_volume = st.number_input("Enter target volume (gallons):", min_value=0.0, max_value=float(volume_gal[-1]), step=1.0)
        ...
        target_liters = st.number_input("Enter target volume (liters):", min_value=0.0, max_value=float(volume_gal[-1]) * 3.78541, step=1.0)
        target_volume_gal = target_liters / 3.78541
        height_in_result = float(height_interp(target_volume_gal))
        st.subheader("📊 Results")
        st.write(f"**Required Height:** {height_in_result * 2.54:.2f} cm")
else:
    st.error("Mismatch in data lengths between height and volume arrays. Please verify your input data.")
